...
STATIC UINT32             mNvFlashBase;
STATIC UINT32             mNvFlashSize;
STATIC UINT32             mNvFlashBlockSize;
  ...
  )
{
  ...
  TotalNvStorageBlocks = mNvStorageSize / mNvFlashBlockSize;
  ...
  )
{
  ...
  if (Offset + *NumBytes > mNvFlashBlockSize) {
    return EFI_BAD_BUFFER_SIZE;
  }

  Status = mSpiFlash->FlashRead (mSpiFlash, FlashRegionNv,
             Lba * mNvFlashBlockSize + Offset,
             *NumBytes,
             Buffer
             );

  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to do flash read Lba %d Offset 0x%X Count %d\n", 
                __FUNCTION__, Lba, Offset, *NumBytes));
  ...
  )
{
  ...
  Status = mSpiFlash->FlashWrite (mSpiFlash, FlashRegionNv,
             Lba * mNvFlashBlockSize + Offset,
             *NumBytes,
             Buffer
             );
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to do flash write Lba %d Offset 0x%X Count %d\n", 
                __FUNCTION__, Lba, Offset, *NumBytes));
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
       ...
       Start = VA_ARG (Args, EFI_LBA))
  {
    ...
    Status = mSpiFlash->FlashErase (mSpiFlash, FlashRegionNv,
                  Start * mNvFlashBlockSize,
                  Length * mNvFlashBlockSize
                  );
    if (EFI_ERROR (Status)) {
      DEBUG ((DEBUG_ERROR, "%a: Failed to do flash erase Offset 0x%X Count %d\n", 
                  __FUNCTION__, Start * mNvFlashBlockSize, Length * mNvFlashBlockSize));
      break;
    }
  }
  VA_END (Args);
  ...
  )
{
  ...
  mNvFlashBlockSize = FixedPcdGet32 (PcdFlashPageSize);
  ...
  Status = mSpiFlash->FlashGetRegion (mSpiFlash, FlashRegionNv, &mNvFlashBase, &mNvFlashSize);
  if (EFI_ERROR (Status)) {
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to install Firmware Volume Block protocol\n", __FUNCTION__));
  }
