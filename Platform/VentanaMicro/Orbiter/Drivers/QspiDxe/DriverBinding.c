  ...
  )
{
  ...
  if (CompareGuid (Dev->Type, &gOrbiterNonDiscoverableRuntimeSysQspiMasterGuid)) {
    Status = EFI_SUCCESS;
  } else {
...
EFI_STATUS
EFIAPI
QspiDriverBindingStart (
  IN EFI_DRIVER_BINDING_PROTOCOL  *This,
  IN EFI_HANDLE                   ControllerHandle,
  IN EFI_DEVICE_PATH_PROTOCOL     *RemainingDevicePath OPTIONAL
  )
{
  return QspiInit (This->DriverBindingHandle, ControllerHandle);
}
...
{
  return QspiRelease (This->DriverBindingHandle, ControllerHandle);
}


EFI_DRIVER_BINDING_PROTOCOL  gQspiDriverBinding = {
  QspiDriverBindingSupported,
  QspiDriverBindingStart,
  QspiDriverBindingStop,
  0xa,
...
/**
  The entry point of Qspi UEFI Driver.
  ...
  )
{
  ...
  Status = EfiLibInstallDriverBindingComponentName2 (
             ImageHandle, SystemTable, &gQspiDriverBinding, ImageHandle,
             NULL, &gQspiDriverComponentName2);
  ASSERT_EFI_ERROR (Status);

  DEBUG ((DEBUG_INFO, "*** Installed  Qspi UEFI driver! ***\n"));
...
/**
  Unload function for the Qspi UEFI Driver.
