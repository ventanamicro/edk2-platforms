...
{
  EFI_PHYSICAL_ADDRESS FwVarBaseAddress = FixedPcdGet32(PcdFlashNvStorageVariableBase);
  UINT32 FwVarSize = FixedPcdGet32(PcdOvmfFirmwareFdSize) -
                     (FwVarBaseAddress - FixedPcdGet32(PcdOvmfFdBaseAddress));
  ...
  DEBUG ((
    ...
    __func__,
    FwVarBaseAddress,
    FwVarBaseAddress + FwVarSize - 1
     ));

  if (FwVarBaseAddress < SystemMemoryBase ||
      FwVarBaseAddress >= (SystemMemoryBase + SystemMemorySize)) {
    DEBUG ((DEBUG_INFO, "%a: FW is in FLASH\n", __func__));
    ...
    DEBUG ((DEBUG_INFO, "%a: FW is in RAM\n", __func__));
      AddReservedMemoryBaseSizeHob (FwVarBaseAddress, FwVarSize);
      if (FwVarBaseAddress == SystemMemoryBase) {
        AddMemoryRangeHob (FwVarBaseAddress + FwVarSize,
                           SystemMemoryBase + SystemMemorySize);
      } else {
        AddMemoryRangeHob (SystemMemoryBase, FwVarBaseAddress);
        AddMemoryRangeHob (FwVarBaseAddress + FwVarSize,
                           SystemMemoryBase + SystemMemorySize
                          );
      }
