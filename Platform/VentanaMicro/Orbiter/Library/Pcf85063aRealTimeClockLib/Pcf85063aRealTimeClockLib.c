...
#define PCF85063A_DATA_REG_OFFSET 0x4

#define PCF85063A_CLOCK_INVALID   0x80
...
#define PCF85063A_YEAR_MASK       0xff

STATIC EFI_HANDLE                 mI2cMasterHandle;
STATIC VOID                       *mI2cMasterEventRegistration;
STATIC EFI_I2C_MASTER_PROTOCOL    *mI2cMaster;
STATIC EFI_EVENT                  mRtcVirtualAddrChangeEvent;
STATIC UINTN                      mI2cBusFrequency = FixedPcdGet16 (PcdI2cBusFrequency);
  ...
  )
{
  ...
  if ((DateTime.Seconds & PCF85063A_CLOCK_INVALID) != 0) {
    Time->Second  = 0;
    Time->Minute  = 0;
    Time->Hour    = 0;
    Time->Day     = 1;
    ...
    Time->Second  = BcdToDecimal8 (DateTime.Seconds & PCF85063A_SECONDS_MASK);
    Time->Minute  = BcdToDecimal8 (DateTime.Minutes & PCF85063A_MINUTES_MASK);
    Time->Hour    = BcdToDecimal8 (DateTime.Hours & PCF85063A_HOURS_MASK);
    Time->Day     = BcdToDecimal8 (DateTime.Days & PCF85063A_DAYS_MASK);
    Time->Month   = BcdToDecimal8 (DateTime.Months & PCF85063A_MONTHS_MASK);
    Time->Year    = BcdToDecimal8 (DateTime.Years) + RTC_DEFAULT_MIN_YEAR;
  }
  ...
  )
{
  ...
  if (Time->Year < RTC_DEFAULT_MIN_YEAR || Time->Year >= RTC_DEFAULT_MAX_YEAR) {
    return EFI_DEVICE_ERROR;
  }

  Packet.Reg = PCF85063A_DATA_REG_OFFSET;
...
{
  //
  // Currently unimplemented. The PCF85063A does not support setting the alarm
  ...
  // indicates whether the PCF85063A's interrupt line is connected in a way that
...
I2cMasterClaimBus (VOID)
{
  ...
  //
  // FIXME: Prevent concurrent usage of the same I2CMaster
  //
  Status = mI2cMaster->Reset (mI2cMaster);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: mI2cMaster->Reset () failed - %r\n",
      __FUNCTION__, Status));
    return EFI_DEVICE_ERROR;
  }

  Status = mI2cMaster->SetBusFrequency (mI2cMaster, &mI2cBusFrequency);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "%a: mI2cMaster->SetBusFrequency () failed - %r\n",
      __FUNCTION__, Status));
    return EFI_DEVICE_ERROR;
  }
  ...
  )
{
  ...
  Status = gBS->LocateHandle (ByProtocol,
                  &gPcf85063aRealTimeClockLibI2cMasterProtocolGuid, NULL,
