...
{
  return (VOID *)(FixedPcdGet64 (PcdTemporaryRamBase64) + FixedPcdGet32 (PcdTemporaryRamSize) - SEC_HANDOFF_DATA_RESERVE_SIZE);
}
  ...
  )
{
  ...
  if (FixedPcdGetBool (PcdRiscVPeiBootEnable)) {
    SecStartup (TempRamSize, TempRamBase, (VOID *)(UINTN)PcdGet64 (PcdRiscVDxeMemFvBase64));
  } else {
    // Default PEIless booting
    SecPeilessStartup (TempRamSize, TempRamBase, (VOID *)(UINTN)PcdGet64 (PcdRiscVDxeMemFvBase64));
  }
