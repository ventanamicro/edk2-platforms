...
uint64_t run_x86_func(void *func, uint64_t *args)
{
    ...
    for(;;) {
        ...
        if (trapnr == EXCP_RETURN_TO_NATIVE) {
            ...
            printf_verbose("XXX  Calling native %p(%llx, %llx, %llx, %llx, %llx, %llx, %llx, %llx)\n",
                           f, env->regs[R_ECX], env->regs[R_EDX], env->regs[8],
                           env->regs[9], stackargs[5], stackargs[6], stackargs[7],
                           stackargs[8]);
#ifndef MDE_CPU_RISCV64
            assert(!(env->eip & 0x3)); /* Make sure we're calling native code which is aligned */
#endif
            ...
            printf_verbose("XXX  Finished native call to %p (return to %lx)\n", f, stackargs[0]);
            env->eip = stack_pop64();
        } else if (trapnr == EXCP_HLT) {
            printf_verbose("XXX  EXCP_HLT\n");
            env->halted = 0;
        } else {
